...
export default function PlayerOverlayPage() {
  ...
  const [scopeType, setScopeType] = useState<'default' | 'set' | 'unsold'>('default');
  const [currentSetId, setCurrentSetId] = useState<string | null>(null);
  const [scopePlayers, setScopePlayers] = useState<any[]>([]);
  ...
  async function fetchCurrent() {
    ...
    // scope label
    try {
      const scope = ((a as any)?.queue_scope || 'default') as 'default' | 'set' | 'unsold';
      const setId = ((a as any)?.current_set_id || null) as string | null;
  ...
  // Load players for the selected set/scope
  useEffect(() => {
    let cancelled = false;
    (async () => {
      let q = supabase
        ...
        .limit(200);
      if (scopeType === 'unsold') q = q.eq('status', 'unsold');
      if (scopeType === 'set' && currentSetId) q = q.eq('set_id', currentSetId);
      const { data } = await q;
      if (cancelled) return;
      setScopePlayers((data as any[]) || []);
    })();
    return () => { cancelled = true; };
  }, [auctionId, scopeType, currentSetId]);
          ...
          <main className="flex-1 flex items-center justify-center">
            <div className="w-full max-w-6xl px-4">
              <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden border-2 border-white">
                <div className="grid gap-0" style={{ gridTemplateColumns: '1fr 2fr 240px' }}>
                  ...
                  {/* Right column: Players in current set/scope (grid view) */}
                  <div className="col-span-1 p-4 bg-[#F8F7FA]">
                    <div className="mb-3 text-sm font-bold text-gray-600 uppercase tracking-wider">{scopeLabel}</div>
                    <div
                      className="grid grid-cols-2 gap-x-6 gap-y-3 max-h-[475px] overflow-auto pr-1"
                      style={{
                        gridTemplateRows: 'repeat(5, 85px)',
                        gridTemplateColumns: 'repeat(2, 85px)',
                        justifyContent: 'center',
                      }}
                    >
                      {scopePlayers.map((p: any, idx: number) => {
                        const url = p.photo_path ? supabase.storage.from('player-photos').getPublicUrl(p.photo_path).data.publicUrl : (p.photo_url || '');
                        const isDim = String(p.status || '') !== 'available';
                        return (
                          <div key={p.id} className="relative rounded-lg border border-gray-100 bg-gray-50 shadow-sm overflow-hidden">
                            {/* image */}
                            ...
                            {/* top-left serial number (replacing category icon) */}
                            <div className="absolute left-1 top-1 h-5 w-5 rounded-full bg-white/90 shadow flex items-center justify-center">
                              <span className="text-[10px] font-semibold text-gray-700 leading-none">{idx + 1}</span>
                            </div>
                            {/* status overlay and pill */}
                            {isDim && <div className="absolute inset-0 bg-white/60" />}
                            {(p.status === 'sold' || p.status === 'unsold') && (
                              <div className="absolute right-1 top-1 flex">
                                <span className={`inline-flex items-center justify-center rounded-full px-2 py-0.5 text-[10px] font-semibold border ${p.status === 'sold' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>
                                  {p.status === 'sold' ? 'S' : 'U'}
                                </span>
                              </div>
                            )}
                            {/* name */}
                            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-white/95 to-white/10 px-2 py-1">
                              <div className="truncate text-[12px] font-semibold text-gray-800" title={p.name}>{p.name}</div>
                            </div>
                          </div>
                        );
                      })}
                      {scopePlayers.length === 0 && (
                        <div className="col-span-2 rounded-md border border-gray-100 bg-gray-50 px-2 py-2 text-xs text-gray-500">No players in this scope.</div>
                      )}
                    </div>
                  </div>
                </div>
